...
export default function CommitteeDashboardPage() {
  ...
  const updateElectionStatus = async (id: string, newStatus: any) => {
    try {
      ...
      const response = await fetch(
        `http://localhost:3000/elections/${id}/status`,
        {
          method: "PUT",
          credentials: "include",
          headers: {
  ...
  return (
    <DashboardShell>
      <DashboardHeader
        heading="Election Committee Dashboard"
        text="Manage elections and candidates"
      >
          ...
          <Card>
            <CardContent className="py-8">
              <div className="text-center">
                ...
                <Link
                  href="/dashboard/committee/create-election"
            ...
            <Card key={election._id}>
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="flex items-center text-sm">
                    ...
                    <span>
                      Dates: {formatDate(election.startDate)} -{" "}
              ...
              <CardFooter>
                <div className="flex gap-2 w-full">
                  {election.status === "completed" ? <Link href={`/results/${election._id}`} className="flex-1">
                    <Button variant="outline" className="w-full">
                      View Results
                    </Button>
                  </Link> : <Link href={`/election/${election._id}`} className="flex-1">
                    <Button variant="outline" className="w-full">
                      View Details
                    </Button>
                  </Link>}
